...
async function saveTransactionGeneration(data: {
  ...
  duration_ms: number
  client_ip: string
  response_timestamp?: string // Now optional, comes from API response
}) {
  try {
    ...
    const { data: result, error } = await supabase.from("transaction_generations").insert([
      {
        ...
        response_timestamp: data.response_timestamp, // From API response created_at
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    saveTransactionGeneration({
      ...
      client_ip: clientIP,
      response_timestamp: apiCreatedAt, // Use API's created_at field
    }).catch((error) => {
